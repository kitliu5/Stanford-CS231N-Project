...
class cyclegan(object):
    def __init__(self, sess):
        ...
        self.L1_lambda = 10.0
        self.dataset_dir = '/home/kitliu21/crawler'
    ...
    def _build_model(self):
        ...
        self.test_A = tf.placeholder(tf.float32,[None, self.image_size, self.image_size,self.input_c_dim], name='test_A')
        self.test_B = tf.placeholder(tf.float32,[None, self.image_size, self.image_size,self.output_c_dim], name='test_B')
    ...
    def train(self):
        ...
        load_weights(self.saver, self.sess, '/home/kitliu21/Image-Colorization-CycleGAN/weights/horse2zebra_256')
        ...
        for epoch in range(self.epochs):
            dataA = glob(self.dataset_dir + '/PixivImage_train_line/*.jpg')
            dataB = glob(self.dataset_dir + '/PixivImage_train_reduced/*.jpg')
            ...
            for idx in range(0, batch_idxs):
                ...
                if idx % 300 == 0:    
                    Save(self.saver, self.sess, '/home/kitliu21/Image-Colorization-CycleGAN/weights/horse2zebra_256', epoch)
    ...
    def sample_model(self, epoch, idx):
        ...
        dataA = glob(self.dataset_dir + '/PixivImage_val_line/*.jpg')
        dataB = glob(self.dataset_dir + '/PixivImage_val_reduced/*.jpg')
        np.random.shuffle(dataA)
        np.random.shuffle(dataB)
    ...
    def test(self):
        ...
        init_op = tf.global_variables_initializer()
        self.sess.run(init_op)
        sample_files = glob(self.dataset_dir + '/PixivImage_test_line/*.jpg')
        

        if load_weights(self.saver, self.sess, '/home/kitliu21/Image-Colorization-CycleGAN/weights/horse2zebra_256'):
            print(" [*] Load SUCCESS")
        else:
            print(" [!] Load failed...")

        # write html for visual comparison
        index_path = os.path.join('./datasets/test_results/', '{0}_index.html'.format('AtoB'))
        ...
        #in_var = tf.placeholder(tf.float32,[None, self.image_size, self.image_size,1], name='test_A')
        #out_var = self.generator(in_var, self.options, True, name="test_out")

        for sample_file in sample_files:
            print('Processing image: ' + sample_file)
            sample_image = [load_test_data(sample_file, 256)]
            sample_image = np.array(sample_image).astype(np.float32)
            image_path = os.path.join('./datasets/test_results/','{0}_{1}'.format('AtoB', os.path.basename(sample_file)))
